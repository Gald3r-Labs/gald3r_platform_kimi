...
<h1 align="center">Gald3r for Kimi Code (Moonshot)</h1>

<p align="center">
  The <strong>Kimi Code (Moonshot) edition</strong> of Gald3r -- file-based memory, task management, and
  agent orchestration that runs <strong>inside Kimi Code (Moonshot)</strong>. Part of the Gald3r framework
...
This is the **Kimi Code (Moonshot) distribution** of Gald3r. The [`kimi/`](./kimi/) folder is a
complete, ready-to-deploy gald3r setup tuned for Kimi Code (Moonshot) -- drop its contents into your
...
```bash
git clone https://github.com/wrm3/gald3r_platform_kimi.git
cp -r gald3r_platform_kimi/kimi/. /path/to/your/project/
```

Then open the project in Kimi Code (Moonshot) and type `@g-status` to confirm gald3r loaded.

> The `kimi/` payload is **additive** -- it drops in gald3r's config + `.gald3r/` state and
